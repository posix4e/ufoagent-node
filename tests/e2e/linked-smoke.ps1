...
# (command API -> DO push -> agent WS -> execute -> result).
$ErrorActionPreference = 'Stop'
. "$PSScriptRoot/helpers.ps1"
# The login agent's WS loop polls the store every 30s and connects as soon as the token appears.
...
Write-Host 'WS connected - sending a repair command via the control plane API...'
$resp = Send-NodeCommand 'repair'
Write-Host "sent: id=$($resp.id) status=$($resp.status)"
